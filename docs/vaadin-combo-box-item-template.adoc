...
== Material Design with the Item Template
...
[NOTE]
If you use images in the template, make sure they have pre-defined bounds so the layout doesn't break once the images are resized after loading.
The example template below uses [elementname]#paper-icon-item# which does this automatically for it's icon.

[source,html]
----
<dom-module id="my-paper-item">
  <template>
    <style>
      :host {
        display: block;
