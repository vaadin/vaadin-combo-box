...
[NOTE]
Setting the `item-label-path` property is necessary when using object as items, and item templates. This is because the value specified by the `item-label-path` property is displayed in the input field when the item is selected, and it's also used for filtering.
